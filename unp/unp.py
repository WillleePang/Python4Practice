...
def fnmatch(pattern, filename):
    #normacase返回小写path，斜杠变成反斜杠，basename返回文件名和拓展名
...
def get_mimetype(filename):
    ...
    if file_executable is not None:
        rv = subprocess.Popen(['file', '-b', '--mime-type', filename], stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()[0].strip()
        if rv:
            return rv
...
class UnpackerBase(object):
    ...
    def finish_unpacking(self, tmp_dir, dst):
        ...
        fallback_dist = os.path.join(os.path.abspath(dst), basename)

        while os.path.isdir(fallback_dist):
            fallback_dist = increment_string(fallback_dist)
        ...
        if intended_dst is None or os.path.exists(intended_dst):
            os.rename(tmp_dir, fallback_dist)
    ...
    def unpack(self, dst):
        if not self.silent:
            click.echo('unpack "%s" with %s' % (self.filename, self.executable,), fg='yellow')
...
class Unpacker(UnpackerBase):
    ...
    def real_unpack(self, dst):
        ...
        c = subprocess(args, cwd=cwd, **extra)
